...
impl Sequence3Bit {
    ...
    #[getter]
    pub fn as_bytes(&self, py: Python) -> PyResult<Py<PyBytes>> {
        // ✅ Pas de #[getter], argument `py` ajouté
        Ok(PyBytes::new_bound(py, &self.data)?.into()) // ✅ Pas de `self.py()`, et `?` est valide ici
    }
